...
def show_success_dialog(parent, current_time):
    pygame.mixer.init()
    pygame.mixer.music.load("voice/success.mp3")  # Thay đổi đường dẫn đến file âm thanh
    pygame.mixer.music.play()
    # Tạo cửa sổ dialog thành công
    success_dialog = tk.Toplevel(parent)
    success_dialog.title("Thong bao")
    ...
    success_dialog.iconphoto(False, icon)  # Đặt icon cho dialog
    ...
    success_label = tk.Label(header_frame, text="Diem danh thanh cong!", font=("Arial", 14, "bold"))
    success_label.pack(side="left")
    
    # Thêm các dòng thông báo bổ sung
    text1 = tk.Label(success_dialog, text="Ho ten:", font=("Arial", 12))
    text1.pack(anchor="w", padx=20)
    text2 = tk.Label(success_dialog, text="Vai tro:", font=("Arial", 12))
    text2.pack(anchor="w", padx=20)
    text3 = tk.Label(success_dialog, text=f"Thoi gian: {current_time}", font=("Arial", 12))
    ...
    success_dialog.after(4000, lambda: close_dialog(success_dialog))
