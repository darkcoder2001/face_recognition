...
# from notice_login import login_success_dialog,fail_login_dialog,forgot_password_dialog
...
def create_login_dialog():
    global username_entry, password_entry, login_dialog, username_icon, password_icon, login_icon, forgot_icon
...
def check_user_exists(username, email):
    ...
    if server_name and database_name:
        try:
            ...
            print("Error checking user existence:", ex)
            return False
...
def update_password(username, new_password):
    ...
    if server_name and database_name:
        try:
            ...
            cursor = connection.cursor()
            cursor.execute("UPDATE user_manager SET Password = ? WHERE Username = ?", (new_password, username))
            connection.commit()  # Lưu thay đổi
            cursor.close()
            connection.close()
        except pyodbc.Error as ex:
            print("Error updating password:", ex)
...
def login_check(username, password):
    ...
    if server_name and database_name:
        try:
            ...
            cursor.execute("SELECT COUNT(*) FROM user_manager WHERE Username = ? AND Password = ?", (username, password))
            ...
            if count > 0:
                subprocess.Popen(["python", "admin.py"])
                login_dialog.destroy()  # Xóa cửa s�� đăng nhập
            else:
                fail_login_dialog()# Hiển thị dialog thông báo đăng nhập thất bại
        except pyodbc.Error as ex:
            print("Error checking login:", ex)
